...
| 1.5 | Impute missing values | `impute_missing_values()` |
| 2 | Classify lipids | `classify_lipids()`, `annotate_with_lipidmaps()` |
...
| 4.5 | Correct batch effects | `correct_batch_effects()` |
...
Missing values (NA) are common in untargeted lipidomics and arise when a
lipid's signal falls below the instrument's limit of detection (LOD).
Imputation should be applied **before normalisation** so that per-sample
scaling factors are not biased by the proportion of missing values.

```{r impute}
# Inspect missingness in the example data (none here — this is synthetic)
n_miss <- sum(is.na(data_obj$numeric_data))
cat("Missing values in example data:", n_miss, "\n")
...
set.seed(42)
demo_mat <- data_obj$numeric_data
demo_mat[sample(length(demo_mat), size = round(0.05 * length(demo_mat)))] <- NA
cat("Missingness introduced:", sum(is.na(demo_mat)),
    sprintf("(%.1f%%)\n", 100 * mean(is.na(demo_mat))))

# Apply half-minimum imputation (most common in MS lipidomics)
imputed_mat <- impute_missing_values(demo_mat, method = "half_min")
cat("Missing after imputation:", sum(is.na(imputed_mat)), "\n")
```
...
| Method | Best for |
|--------|----------|
| `half_min` | Most lipidomics data; models the LOD (recommended default) |
...
For the remainder of this vignette we use the complete example data without
...
### Optional: LIPID MAPS Database Annotation

`annotate_with_lipidmaps()` queries the LIPID MAPS Structure Database (LMSD)
REST API to enrich each lipid name with its LIPID MAPS ID, exact monoisotopic
mass, molecular formula, and ontology classes. Requires an internet connection;
the function waits 0.3 s between requests to respect the server rate limit.

```{r lipidmaps, eval=FALSE}
# Annotate the first 5 lipids as a demonstration (~2 s with network access)
ann <- annotate_with_lipidmaps(lipid_names[1:5], verbose = TRUE)
print(ann[, c("Lipid", "LipidMaps_ID", "Formula", "ExactMass", "MainClass")])

# Merge with the automatic classification for a fully enriched table
classification_enriched <- merge(classification, ann, by = "Lipid", all.x = TRUE)
head(classification_enriched)
```
...
## Step 4.5 - Batch Effect Correction

When samples were processed across multiple runs, instruments, or operators,
technical batch effects can obscure biological differences. Batch correction
should be applied **after normalisation** and verified with PCA.
...
For the remainder of this vignette we proceed with the uncorrected
normalised data (the example dataset has no batch structure).
