...
#' \dontrun{
#' dl <- load_lipidomics_data_from_df(generate_example_data())
#' visualize_raw_data_improved(dl, "boxplot", "sample")
#' }
visualize_raw_data_improved <- function(data_list,
  ...
  pivot_long_safe <- function(d, id_vars) {
    long <- tidyr::pivot_longer(d, cols = -tidyr::all_of(id_vars),
  ...
  if (identical(view_mode, "sample")) {
    ...
    fill_aes  <- if (group_col_present) ggplot2::aes(x = Sample, y = Intensity,
                                                       fill = Group)
    else ggplot2::aes(x = Sample, y = Intensity)

    color_aes <- if (group_col_present) ggplot2::aes(x = Intensity, color = Group)
    else ggplot2::aes(x = Intensity, color = Sample)
    ...
    if (plot_type == "boxplot") {
      ...
      p <- ggplot2::ggplot(long, fill_aes) +
        ...
        ggplot2::theme_minimal() +
        ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, hjust = 1, size = 8)) +
        ggplot2::labs(title = "Sample-wise Distribution (Violin)",
      ...
      p <- ggplot2::ggplot(long, ggplot2::aes(x = Intensity)) +
        ...
        ggplot2::theme_minimal() +
        ggplot2::labs(title = "Sample-wise Histogram",
                      x = "Intensity", y = "Count")
    }
  ...
  # -------- Lipid view ---------------------------------------------------
    ...
    fill_aes <- if (group_col_present) ggplot2::aes(x = Lipid, y = Intensity,
                                                      fill = Group)
    else ggplot2::aes(x = Lipid, y = Intensity)
    ...
    if (plot_type == "boxplot") {
      p <- ggplot2::ggplot(long, fill_aes) +
        ...
        ggplot2::labs(title = paste("Top", length(top_lipids), "Variable Lipids"),
                      x = "Lipid", y = "Intensity")
      ...
      p <- ggplot2::ggplot(long, fill_aes) +
        ...
        ggplot2::theme_minimal() +
        ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, hjust = 1, size = 6)) +
        ggplot2::labs(title = paste("Top", length(top_lipids), "Variable Lipids (Violin)"),
                      x = "Lipid", y = "Intensity")

    } else if (plot_type == "density") {
      color_aes2 <- if (group_col_present) ggplot2::aes(x = Intensity, color = Group)
      else ggplot2::aes(x = Intensity, color = Lipid)
...
create_pipeline_plot <- function(data_matrix, title = "Normalization Pipeline",
  ...
  df <- if (is.matrix(data_matrix)) as.data.frame(data_matrix, check.names = FALSE)
  else as.data.frame(data_matrix, check.names = FALSE)
  ...
  long <- tidyr::pivot_longer(df, cols = -tidyr::all_of(id_vars),
                              names_to = "Lipid", values_to = "Intensity")
  long$Intensity <- as.numeric(long$Intensity)
  ...
  fill_aes  <- if (group_col_present) ggplot2::aes(x = Sample, y = Intensity, fill = Group)
               else ggplot2::aes(x = Sample, y = Intensity)
  color_aes <- if (group_col_present) ggplot2::aes(x = Intensity, color = Group)
               else ggplot2::aes(x = Intensity, color = Sample)
  ...
  if (plot_type == "boxplot") {
    ...
    p <- ggplot2::ggplot(long, fill_aes) +
      ...
      ggplot2::geom_boxplot(width = 0.1, outlier.alpha = 0.2,
                            fill = "white", color = "grey40") +
      base_theme +
      ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, hjust = 1, size = 6)) +
      ggplot2::labs(title = title, x = "Sample", y = "Intensity")
...
create_volcano_plot_labeled <- function(results,
  ...
  if (!is.null(classification_data) && !is.null(color_by)) {
    ...
    classification_data <- classification_data[
      !duplicated(classification_data$Lipid), , drop = FALSE]
    ...
    if (!color_by %in% colnames(results)) {
      no_match_msg <- paste0("Column '", color_by,
      ...
      if (n_matched == 0 && any(results$IsSignificant)) {
        ex_cls  <- if (nrow(classification_data) > 0)
          classification_data$Lipid[1] else "none"
    ...
    if (!is.null(no_match_msg)) {
      ...
      p <- ggplot2::ggplot(results,
                           ggplot2::aes(x = logFC, y = neg_log10_pval,
                                        color = Significance)) +
        ggplot2::geom_point(alpha = 0.6, size = 2) +
        ggplot2::scale_color_manual(
          values = c(Increased = "red", Decreased = "blue",
      ...
      class_colors   <- if (length(unique_classes) > 0)
        stats::setNames(scales::hue_pal()(length(unique_classes)), unique_classes) else c()
...
create_lipid_expression_barplot <- function(data_matrix,
  ...
  group_vec    <- if (group_column %in% colnames(metadata))
    as.character(metadata[[group_column]]) else rep("Unknown", nrow(metadata))
...
create_plsda_plot_with_ellipses <- function(plsda_data,
  ...
  if (ellipse_type == "confidence") {
    p <- p +
      ...
      ggplot2::stat_ellipse(type = "norm", level = confidence_level,
    ...
    p <- p +
      ggplot2::stat_ellipse(ggplot2::aes(fill = Group),
                            type = "t", level = 0.8,
                            geom = "polygon", alpha = 0.1, color = NA) +
      ggplot2::stat_ellipse(type = "t", level = 0.8,
                            linetype = 2, linewidth = 1, show.legend = FALSE) +
      ggplot2::guides(color = ggplot2::guide_legend(override.aes = list(fill = NA)),
                      fill  = "none")
  }
...
create_enrichment_dotplot <- function(enrichment_data,
  ...
  enrichment_data <- utils::head(enrichment_data[order(enrichment_data$pval), ],
                                 max_pathways)

  ggplot2::ggplot(enrichment_data,
                  ggplot2::aes(x = NES,
