...
#' @examples
#' \dontrun{
#' loaded <- load_lipidomics_data("my_data.csv")
...
classify_lipids <- function(lipid_names) {
  ...
  classify_single <- function(lipid) {
    ...
    data.frame(Lipid = lc, LipidGroup = group, LipidType = type,
...
determine_saturation <- function(lipid_name) {
  ...
  tryCatch({
    ...
    if (length(p3) > 0) {
      db <- sum(vapply(stringr::str_trim(p3),
    ...
    if (!is.na(p4)) {
      ...
      if (!is.na(m[1, 2])) return(classify_db(as.numeric(m[1, 2])))
    }
...
get_normalization_methods <- function() {
  c("TIC", "PQN", "Quantile", "VSN", "Median", "Mean",
...
load_custom_classification <- function(file_path) {
  classification <- utils::read.csv(file_path, check.names = FALSE,
                                    stringsAsFactors = FALSE)
...
#' export_classification(cls, "lipid_classification.csv")
#' }
...
#' @examples
#' \dontrun{
#' sets <- load_custom_enrichment_sets("my_sets.csv")
#' }
